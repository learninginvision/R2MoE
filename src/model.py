...
class CustomDiffusion(LatentDiffusion):
    def __init__(self,
        ...
        if self.freeze_model == 'crossattn-kv':
            ...
            lora_params, _ = inject_trainable_lora(self.model.diffusion_model)
            print("inject_trainable_lora")
            for x in self.model.diffusion_model.named_parameters():
                x[1].requires_grad = False
        
        '''
        for _up, _down in extract_lora_ups_down(self.model.diffusion_model):
