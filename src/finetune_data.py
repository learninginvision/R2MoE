...
class MaskBase(Dataset):
    def __init__(self,
        ...
        self.aug = aug
        self.datapath_mask=datapath_mask
        self.repeat = repeat
        self.style = style
        self.task_id=task_id
        self.templates_small = templates_small# 'photo of a {}'
        if self.style:
            self.templates_small = templates_small_style# 'painting in the style of {}'
        if os.path.isdir(datapath):
            self.image_paths1 = [os.path.join(datapath, file_path) for file_path in os.listdir(datapath) if isimage(file_path)]#images_path 每张图片的路径
        else:
            with open(datapath, "r") as f:
                self.image_paths1 = f.read().splitlines()
        
            
        self._length1 = len(self.image_paths1)#图片数量
        self.image_paths2 = []
        self._length2 = 0
        if reg_datapath is not None:
            if os.path.isdir(reg_datapath):
                self.image_paths2 = [os.path.join(reg_datapath, file_path) for file_path in os.listdir(reg_datapath) if isimage(file_path)]#正则化图片的路径
            else:
                with open(reg_datapath, "r") as f:
                    self.image_paths2 = f.read().splitlines()
            self._length2 = len(self.image_paths2)#正则化图片数量
        self.image_paths3=[]
        self._length3=0
        if datapath_mask is not None:
            with open(datapath_mask, "r") as f:
                self.image_paths3 = f.read().splitlines()
            self._length3 = len(self.image_paths3)
        
        
        self.labels = {
            "relative_file_path1_": [x for x in self.image_paths1],
            "relative_file_path2_": [x for x in self.image_paths2],
            "relative_file_path3_": [x for x in self.image_paths3],
        }#路径
    ...
    def __getitem__(self, i):
        ...
        if i > self._length2 or self._length2 == 0:
                #print(self.labels["relative_file_path1_"][i % self._length1])
                image = Image.open(self.labels["relative_file_path1_"][i % self._length1])
                if self.datapath_mask:
                    concept_mask=Image.open(self.labels["relative_file_path3_"][i % self._length1])
                    
                else:
                    width, height = image.size
                    concept_mask= Image.new("RGB", (width, height), color=(255, 255, 255))
                if isinstance(self.caption, str):
                    ...
                    example["caption"] =np.random.choice(self.templates_small).format(self.caption[i % min(self._length1, len(self.caption)) ])
                #print("caption",example["caption"])
                #print("image_path",self.labels["relative_file_path1_"][i % self._length1])
            
        else:
            image = Image.open(self.labels["relative_file_path2_"][i % self._length2])
            #print(self.labels["relative_file_path2_"][i % self._length2])
            width, height = image.size
            concept_mask= Image.new("RGB", (width, height), color=(255, 255, 255))
            if isinstance(self.reg_caption, str):
                example["caption"] = np.random.choice(self.templates_small).format(self.reg_caption)
                #example["caption"]=self.reg_caption
            else:
                example["caption"] = self.reg_caption[i % self._length2]
        #print("caption",example["caption"])
        if not image.mode == "RGB":
            image = image.convert("RGB")
        if not concept_mask.mode == "RGB":
            concept_mask = concept_mask.convert("RGB")
        # default to score-sde preprocessing
        img = np.array(image).astype(np.uint8)
        concept_mask1 = np.array(concept_mask).astype(np.uint8)
        crop = min(img.shape[0], img.shape[1])
        h, w, = img.shape[0], img.shape[1]

        img = img[(h - crop) // 2:(h + crop) // 2,
                  (w - crop) // 2:(w + crop) // 2]
        concept_mask1 = concept_mask1[(h - crop) // 2:(h + crop) // 2,
                  (w - crop) // 2:(w + crop) // 2]
        concept_mask=Image.fromarray(concept_mask1)
        concept_mask=self.flip(concept_mask)
        ...
        if i > self._length2 or self._length2 == 0:
            ...
            if random_scale < 0.6*self.size:
                ...
                concept_mask = concept_mask.resize((random_scale, random_scale), resample=Image.NEAREST)
                image = np.array(image).astype(np.uint8)
                concept_mask= np.array(concept_mask).astype(np.uint8)
                image = (image / 127.5 - 1.0).astype(np.float32)
                #concept_mask=(concept_mask / 127.5 - 1.0).astype(np.float32)
                ...
                input_mask1[cx - random_scale // 2: cx + random_scale // 2, cy - random_scale // 2: cy + random_scale // 2, :] = concept_mask
                input_mask2 = Image.fromarray(input_mask1)
                #print("input_mask1.shape",input_mask1.shape)
                resized_image = input_mask2.resize((self.size // 8, self.size // 8), resample=Image.NEAREST)
                
                input_mask1 = np.array(resized_image).astype(np.uint8)
                input_mask1=(input_mask1 / 255.0).astype(np.float32)
                ...
                input_mask1  = concept_mask[cx - self.size // 2: cx + self.size // 2, cy - self.size // 2: cy + self.size // 2, :]
                ...
                if self.size is not None:
                    ...
                    concept_mask =concept_mask.resize((self.size//8, self.size//8), resample=Image.NEAREST)
                input_image1 = np.array(image).astype(np.uint8)
                input_mask1 = np.array(concept_mask).astype(np.uint8)
                #input_mask1 = np.array(concept_mask).astype(np.uint8)
        ...
        example["concept_mask"] = np.sum(input_mask1, axis=2)
        example["concept_mask"] = np.where(example["concept_mask"] > 0, 1, 0)
        
        example["mask"] = mask
        example["task_id"]=self.task_id
