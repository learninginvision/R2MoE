...
def compose(paths, category, outpath, pretrained_model_path, regularization_prompt, prompts, save_path, device='cuda'):
    ...
    #layers_modified包含了所有要融合的层的名称
    layers = []
    layers_modified = []
    layers_modified_lora=[]
    def getlayers(model, root_name=''):
        for name, module in model.named_children():#返回包含子模块的迭代器，同时产生模块的名称以及模块本身。
    ...
    for path1, cat1 in zip(paths.split('+'), category.split('+')):
        model2_st = torch.load(path1,map_location=device)#第一个concept的模型
        if 'embed' in model2_st['state_dict']:
            ...
            embeds.append(model2_st['state_dict']['embed'][-1:])#添加embed的最后一个向量
    ...
    embeds = torch.cat(embeds, 0).to(device)
    ...
    for composing_model_count in range(len(model2_sts)):
        ...
        for each in layers_modified:
            each1=  each.replace('weight','lora_up.weight') 
            print("each1",each1)
            each2=  each.replace('weight','lora_down.weight') 
            print("each2",each2)
            #uc_values[each].append((model2_sts[composing_model_count][each].to(device)@uc_targets[-1].T).T)#
            model2_sts[composing_model_count][each]=((model2_sts[composing_model_count][each1]@model2_sts[composing_model_count][each2])+model.state_dict()[each]).to(device)
            uc_values[each].append((model2_sts[composing_model_count][each].to(device)@uc_targets[-1].T).T)#
    ...
    torch.save({'state_dict': new_weights}, f'{save_path}/{outpath}/checkpoints/composenW_lora.ckpt')
