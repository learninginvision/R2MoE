...
class FrozenCLIPEmbedderWrapper(AbstractEncoder):
    ...
    def __init__(self, modifier_token, version="/data1/gxh/dataroot/models/openai/clip-vit-large-patch14", device="cuda", max_length=77):
        super().__init__()
    ...
    def add_token(self):
        ...
        for i in range(len(self.modifier_token_id)):
            token_embeds[self.modifier_token_id[-(i+1)]] = torch.nn.Parameter(token_embeds[42170].clone(), requires_grad=True)
            
        # if len(self.modifier_token_id)==5:
        #     token_embeds[self.modifier_token_id[-1]] = torch.nn.Parameter(token_embeds[48136], requires_grad=True)
        ...
        #     token_embeds[self.modifier_token_id[-4]] = torch.nn.Parameter(token_embeds[48172], requires_grad=True)
        #     token_embeds[self.modifier_token_id[-5]] = torch.nn.Parameter(token_embeds[42170], requires_grad=True)
        # elif len(self.modifier_token_id)==1:
        #      token_embeds[self.modifier_token_id[-1]] = torch.nn.Parameter(token_embeds[48136], requires_grad=True)

 
    def loss_ortho_text(self, task_id=None,loss_type="FA",device=None):
        embedding_all=torch.cat([self.transformer.text_model.embeddings.token_embedding[-5:-5+task_id].detach(),self.transformer.text_model.embeddings.token_embedding[-5+task_id]], dim=0)
        QQT = torch.mm(embedding_all, embedding_all.t())
        if loss_type=="FA1":
    ...
    def forward(self, text):
        ...
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")#输出token_id
        tokens = batch_encoding["input_ids"].to(self.device)#得到输入的tokens_id
        if torch.max(tokens) >49407:
            self.position= torch.nonzero(tokens > 49407, as_tuple=False)
            self.position =self.position[:, 1]
        indices = tokens == self.modifier_token_id[-1]
        
        #max_token_id=torch.max(tokens)
        #if max_token_id>min(self.modifier_token_id):
        #    self.transformer.get_input_embeddings().weight.data[max_token_id-1].requires_grad=False
